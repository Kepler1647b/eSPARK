...
from mymodels import GLU, Attn_Net_Gated, CLAM_SB, TransformerBlocks, TransformerBlocksCross, batch_CLAM_SB, CLAM_SB_multimodal2, CLAM_SB_multimodal_mul2
import sys
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(train_cases_all, train_label)):
    ...
    optimizer = optim.AdamW([{'params': model.preprocess_ct.parameters(), 'lr': model_hyperparameters["lr"]*0.01}, {'params': list(model.attention_net.parameters())+list(model.classifiers.parameters()) + list(model.multi_modal_classifiers.parameters()), 'lr': model_hyperparameters["lr"]}], weight_decay=model_hyperparameters["weight_decay"])
    ...
    for epoch in tqdm(range(model_hyperparameters["num_epochs"]), colour="green"):
        ...
        csv_path = os.path.join(comment, "csv", str(fold))
